...
// Vertical Response Plugin WIDGET // Vertical Response Plugin WIDGET - custom code - RA
    function widget_vr($args = false, $echo = true) {
        ...
        if (empty($options['showname']) || !isset($options['showname'])) {
            $options['showname'] = 'full';
        };

        $output .= $before_widget;
       
        $output .= '<form method="post" action="http://oi.vresp.com?fid=' . $options['code'] . '" target="vr_optin_popup" onsubmit="window.open( \'http://www.verticalresponse.com\', \'vr_optin_popup\', \'scrollbars=yes,width=600,height=300\' ); return true;" id="vr_form">';

        $output .= '<div class="subscribe_wrapper">';

        $output .= '<table>';
        $output .= '<tr><td class="subscribe_header">';
        ...
        $output .= '<tr><td class="subscribe_body" align="right">';
            $output .= '<input type="text" style="width:205px;"  class="subscribe_input" id="email_address" name="email_address" value="Enter your email address"  onfocus="if (this.value == \'Enter your email address\') {this.value = \'\';}" onblur="if (this.value == \'\') {this.value = \'Enter your email address\';}"';
            $output .= '/>';
        
            $output .= '</td></tr>';

            $output .= '<tr><td class="subscribe_footer" align="right">';
        $output .= '<input type="hidden" name="vr_widget_version" value="' . $kwd_vr_version . '" />';

        $output .= '<input type="submit" class="subscribe_button" value="" ';
...
// If you have chosen to show the attribution link, add the link
        if ($options['credit'] != 'no') {
            if (function_exists('add_action') && function_exists('vr_attr')) {
                add_action('wp_footer', 'vr_attr');
            }
        } else {
            if (function_exists('add_action') && function_exists('vr_attr')) {
                add_action('wp_footer', 'get_vr_attr');
            }
        } // End Credit
    }

// Vertical Response Plugin WIDGET - original code
    function widget_vr_old($args = false, $echo = true) {
        global $kwd_vr_version;
        $output = '';
        if ($args) {
            extract($args);
...
<form method="post" action="http://oi.vresp.com?fid=' . $options['code'] . '" target="vr_optin_popup" onsubmit="window.open( \'http://www.verticalresponse.com\', \'vr_optin_popup\', \'scrollbars=yes,width=600,height=450\' ); return true;" id="vr_form">';
        $style = false;
        $required = false;
        if ($options['required'] != 'no') {
            $required = true;
        }
        $output .= ' <div class="vr_wrapper"';
        if ($options['defaultstyle'] != 'no') {
            $style = true;
        ...
        if ($options['preface'] != '') {
            ...
            if ($style) {
                $output .= ' style="padding:0 0 0.5em 0; margin:0; color: #' . $kwd_font_color . '; line-height:1.2;"';
            } $output .= ' class="vr_preface">' . htmlspecialchars(stripslashes($options['preface'])) . '</' . vr_default_string($options['wrap'], 'p') . '>';
        }
        $output .= '
<fieldset';
        if ($style) {
...
<label for="email_address"';
            if ($style) {
                $output .= ' style="color: #' . $kwd_label_color . '; clear:both; width:100%; float:left;"';
            }
//$output .= ' id="email_address_label">Email:';
            $output .= ' id="email_address_label"><img src="/wp-content/themes/PM2/cust_images/Email_List_Button.png" />';
            if ($required) {
                $output .= ' <em id="vr_email_required"';
                if ($style) {
...
<input type="text" id="email_address" name="email_address" size="20" value="Enter your email address"  onfocus="if (this.value == \'Enter your email address\') {this.value = \'\';}" onblur="if (this.value == \'\') {this.value = \'Enter your email address\';}"';
            ...
            if ($style) {
                $output .= '<div style="width:100%;"></div>';
            }
        }
	...
	<label for="first_name"';
            if ($style) {
                $output .= ' style="color: #' . $kwd_label_color . ';"';
            }
...
//$output .= '<input type="submit" value="' . vr_default_string(htmlspecialchars(stripslashes_deep($options['button'])), 'Subscribe') . '"';
        ...
        $output .= '<input type="image" src="/wp-content/themes/PM2/cust_images/Subscribe_Button.png" value="' . vr_default_string(htmlspecialchars(stripslashes_deep($options['button'])), 'Subscribe') . '"';
        ...
        if ($options['show_vr_code'] == 'yes' || $options['show_vr_code'] == '') {
            ...
            $output .= '<span';
            if ($style) {
                $output .= ' style="color: #' . $kwd_font_color . '; line-height:1.1; display:block; padding-top:.5em;"';
            }
...
?>
    ...
    <input type="radio" id="vr_defaultstyle_yes" name="vr_defaultstyle" value="yes" <?php
        if (htmlspecialchars(stripslashes($options['defaultstyle'])) == 'yes' || htmlspecialchars(stripslashes($options['defaultstyle'])) == '') {
            echo 'checked="checked"';
        };
?> />
        <label for="vr_style_no">No
        </label>
        <input type="radio" id="vr_style_no" name="vr_defaultstyle" value="no" <?php
        if (htmlspecialchars(stripslashes($options['defaultstyle'])) == 'no') {
            echo 'checked="checked"';
        };
?> />
        <br />
...
<input type="radio" id="vr_showname_yes" name="vr_showname" value="full" <?php
        if (htmlspecialchars(stripslashes($options['showname'])) == 'full' || htmlspecialchars(stripslashes($options['showname'])) == '') {
            echo 'checked="checked"';
        }; ?> />
        <label for="vr_showname_yes"><strong>Full Optin</strong> <br />(will display First Name, Last Name, Email, and subscribe button)</label>

        <br />
        <input type="radio" id="vr_showname_email" name="vr_showname" value="email" <?php
        if (htmlspecialchars(stripslashes($options['showname'])) == 'email') {
            echo 'checked="checked"';
        }; ?> />
        <label for="vr_showname_email"><strong>Only Email</strong> <br />(will show Email & subscribe button)
        </label>
        <br />
        <input type="radio" id="vr_showname_button" name="vr_showname" value="button" <?php
        if (htmlspecialchars(stripslashes($options['showname'])) == 'button') {
            echo 'checked="checked"';
        };
?> />
 ...
 <input type="radio" id="vr_required_yes" name="vr_required" value="yes" <?php
        if (htmlspecialchars(stripslashes($options['required'])) == 'yes' || htmlspecialchars(stripslashes($options['required'])) == '') {
            echo 'checked="checked"';
        };
?> />
        <label for="vr_style_no">No
        </label>
        <input type="radio" id="vr_required_no" name="vr_required" value="no" <?php
        if (htmlspecialchars(stripslashes($options['required'])) == 'no') {
            echo 'checked="checked"';
        };
?> />
 ...
 <input type="radio" id="vr_show_vr_code_yes" name="vr_show_vr_code" value="yes" <?php
        if (htmlspecialchars(stripslashes($options['show_vr_code'])) == 'yes' || htmlspecialchars(stripslashes($options['show_vr_code'])) == '') {
            echo 'checked="checked"';
        };
?> />
        <label for="vr_show_vr_code_no">No
        </label>
        <input type="radio" id="vr_show_vr_code_no" name="vr_show_vr_code" value="no" <?php
        if (htmlspecialchars(stripslashes($options['show_vr_code'])) == 'no') {
            echo 'checked="checked"';
        }; ?> />
 ...
 <input type="radio" id="vr_credit_yes" name="vr_credit" value="yes" <?php
        if (htmlspecialchars(stripslashes($options['credit'])) == 'yes' || htmlspecialchars(stripslashes($options['credit'])) == '' || !$options['credit']) {
            echo 'checked="checked"';
        }; ?> />
        <label for="vr_credit_no">No
        </label>
        <input type="radio" id="vr_credit_no" name="vr_credit" value="no" <?php
        if (htmlspecialchars(stripslashes($options['credit'])) == 'no') {
            echo 'checked="checked"';
        }; ?> />
 </p>
